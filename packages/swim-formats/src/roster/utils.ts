...
export function parseGenderCode(code: string): "male" | "female" {
...
export function genderFromEventCode(
  code: string,
...
export function parseSdifBirthDate(raw: string): string | undefined {
 ...
 */
export function parseDobFromUsaMemberId(usaId: string): string | undefined {
...
export function parseAusaBirthDate(line: string): string | undefined {
...
export function parseUsaMemberIdFromLine(line: string): string | undefined {
  const match = line.match(/\b([A-F0-9]{14})AUSA/i);
  return match?.[1];
}

const CLASS_AGE: Record<string, number> = {
  FR: 14,
  SO: 15,
  JR: 16,
  SR: 17,
...
export function splitSwimmerName(fullName: string): {
  firstName: string;
  lastName: string;
} {
...
export function parseLastFirstName(name: string): {
  firstName: string;
  lastName: string;
  middleName?: string;
} {
...
export function mergeSwimmer(
  map: Map<string, ParsedRosterRow>,
...
export function extractSeasonYear(content: string): number | undefined {
